...
function problemA() {
  ...
  Promise.all([p1, p2])
    .then((stanza) => {
      stanza.forEach((texto) => blue(texto));
    })
...
function problemB() {
  ...
  // promise version
  Promise.all(
    filenames.map((nombre) => {
      return promisifiedReadFile(nombre);
    })
  )
    .then((stanza) => {
      stanza.forEach((texto) => blue(texto));
...
function problemC() {
  ...
  // promise version
  const promesas = filenames.map((nombre) => {
    return promisifiedReadFile(nombre);
  });

  Promise.allSettled(promesas)
    .then((response) => response.forEach((stanza)=> blue(stanza._settledValueField)))
    .finally(() => console.log("done"));
}
...
function problemD() {
  ...
  .then((response) => response.forEach((stanza)=> blue(stanza)),((error)=>magenta(new Error(error))))
  .finally(() => console.log("done"));
